...
@Component({
  selector: 'true-grid-header-mat',
  templateUrl: './grid-header-mat.component.html',
  styleUrls: ['./styles/grid-header-mat.behavior.scss']
})
export class GridHeaderMatComponent extends BaseComponent {
  ...
  headerCellMouseDown(e: any, col: Column) {
    ...
    if (this.uiAction !== null) {
      // Уже считаем клик или перетаскивание
      return;
    }
  ...
  checkReordering(xx: number, yy: number): boolean {
    if (this.uiAction.action === UIActionType.CLICK && this.state.settings.columnReorder) {
      if (Math.abs(xx - this.uiAction.x) > 20 || Math.abs(yy - this.uiAction.y) > 10) {
  ...
  _getHostElement(col: Column): HTMLElement {
    return null;
